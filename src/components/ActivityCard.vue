<template>
  <v-card fluid shaped outlined class="tab-bar" elevation="12">
    <template slot="progress">
      <v-progress-linear color="gold" height="10" indeterminate></v-progress-linear>
    </template>
    ...
    <v-card-text>
      <v-row align="center" class="mx-0 time">
        <v-chip label outlined text-color="#1976d2" color="#1976d2">{{startTime}}</v-chip>
        <span class="hyphen">-</span>
        <v-chip label outlined text-color="#1976d2" color="#1976d2">{{endTime}}</v-chip>
      </v-row>
      <v-row align="center" class="mx-0">
        <div>{{description}}</div>
      </v-row>
      <v-row align="center" class="mx-0" v-for="(link, index) in links" :key="index">
